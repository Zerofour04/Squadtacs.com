...
function ShowDateRangeSelectionDialog()
{
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Veröffentlichungsdatum' } ) );
	var createdContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Zwischen' } ) );
	...
	createdContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Und' } ) );
	...
	content.append( $J( '<div/>', { class: 'title', text: 'Aktualisierungsdatum'} ) );
	var updatedContainer = $J( '<div/>', { class: 'date_range_filter_dates' } );
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Zwischen' } ) );
	...
	updatedContainer.append( $J( '<div/>', { class: 'date_range_word', text: 'Und' } ) );
	...
	var dialog = ShowConfirmDialog( 'Nach Datum filtern', content );
...
function OnIsCompanyChange()
{
	if ( $('iscompany').checked )
	{
		...
		$('firstnamelabel').innerHTML = 'Firma/Geschäftsbezeichnung';
	}
	else
	{
		$('lastnameblock').style.display = 'block';
		$J('uscitizenblock').show();
		$('firstnamelabel').innerHTML = 'Vorname (wie in Ihren Bankunterlagen)';
	}
}
...
function UpdateBankInfo()
{
	...
	try
	{
				...
				if ( $('bankcountry').value == 'US' )
		{
			...
			$('bankroutingnumberlabel').innerHTML = 'Bankleitzahl';
		}
		else
		{
		...
		try
	{
				...
				if ( $J( "#firstname").length != 0 )
		{
			if ( $( 'iscompany' ).checked )
			{
				...
				{
					errorString += 'Bitte geben Sie eine Firma/Geschäftsbezeichnung ein.<br/>';
				}
				$( 'lastname' ).value = '';
			}
			else
			{
				if ( !IsValidRequiredField( "#firstname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Bitte geben Sie einen gültigen Vornamen ein.<br/>';
				}
				if ( !IsValidRequiredField( "#lastname", gValidFieldAlphaNumericRegex ) )
				{
					errorString += 'Bitte geben Sie einen gültigen Nachnamen ein.<br/>';
				}
			}
		}
		...
		{
			errorString += 'Bitte geben Sie Ihre gültige Adresse ein.<br/>';
		}
		if ( !IsValidRequiredField( "#city", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Bitte geben Sie Ihren gültigen Wohnort ein.<br/>';
		}
		if ( $('country').value == 'US' || $('country').value == 'UA' )
		{
			if ( !IsValidRequiredField( "#zip", gValidFieldAlphaNumericRegex ) )
			{
				errorString += 'Bitte geben Sie Ihre gültige Postleitzahl ein.<br/>';
			}
		}
		if ( $( 'phone' ).value.length < 1 )
		{
			errorString += 'Bitte geben Sie Ihre Telefonnummer inklusive Vorwahl ein.<br/>';
		}
	}
	catch(e)
	{
		...
		else
		{
			...
			$J.ajax( {
				...
				success : function( response ) {
					var dialog = ShowAlertDialog( 'Gespeichert!', response.redirect_url.length != 0 ? 'Ihre Kontakt- und Adressdaten wurden gespeichert. Da Sie Ihr Land aktualisiert haben, werden Sie nun zu einem externen Dienstleister weitergeleitet, um Ihre steuerrechtlichen Angaben zu aktualisieren.' : 'Ihre Kontakt- und Adressinformationen wurden gespeichert. Falls noch nicht geschehen, füllen Sie bitte Ihre Bankverbindung und steuerrechtlichen Angaben aus.' );
					dialog.done( function() {
				...
				error: function( jqXHR ) {
					...
					{
						ShowAlertDialog( 'Fehler', json.msg );
					}
					else
					{
						ShowAlertDialog( 'Fehler', 'Beim Speichern Ihrer Kontaktdaten ist ein unbekannter Fehler aufgetreten.' );
					}
				}
			} );
		}
...
function validateFields()
{
		...
		try
	{
		...
		{
			errorString += 'Bitte geben Sie Ihre gültige E-Mail-Adresse ein.<br/>';
		}
		...
		{
			errorString += 'Bitte geben Sie den gültigen Namen Ihres Kreditinstituts ein.<br/>';
		}
		if ( !IsValidRequiredField( "#bankaccountholdername", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Bitte geben Sie den gültigen Namen des Bankkontoinhabers ein (muss mit den Bankdaten übereinstimmen).<br/>';
		}

				if ( !IsValidRequiredField( "#bankaddress1", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Bitte geben Sie die gültige Adresse Ihres Kreditinstituts ein.<br/>';
		}
		if ( !IsValidRequiredField( "#bankcity", gValidFieldAlphaNumericRegex ) )
		{
			errorString += 'Bitte geben Sie den gültigen Ort Ihres Kreditinstituts ein.<br/>';
		}
		if ( $('bankcountry').value == 'US' || $('bankcountry').value == 'UA' )
		{
			if ( $( 'bankzip' ).value.length < 1 )
			{
				errorString += 'Bitte geben Sie die gültige Postleitzahl Ihres Kreditinstituts ein.<br/>';
			}
		}

				if ( $( 'bankaccountnumber' ).value.length < 1 && !IsIBANCountry( $('bankcountry').value ) )
		{
			errorString += 'Bitte geben Sie Ihre Kontonummer ein.<br/>';
		}
		if ( $( 'bankcountry' ).value == 'US' )
		{
			$('bankiban').value = '';
			...
			{
				errorString += 'Bitte geben Sie Ihre Bankleitzahl ein (9 Ziffern).<br/>';
			}
		}
		else if ( $( 'bankcountry' ).value == 'CA' )
		{
			...
			if ( $( 'bankaccountroutingnumber' ).value.length != 9 && $( 'bankaccountroutingnumber' ).value != $( 'storedroutingnumber' ).value)
			{
				errorString += 'Bitte geben Sie die "Canadian Transit Number" ein (9 Stellen mit einer führenden Null).<br/>';
			}
		}
		else if ( IsIBANCountry( $('bankcountry').value ) )
		{
			...
			{
				errorString += 'Bitte geben Sie Ihre IBAN ein.<br/>';
			}
			if ( $( 'swiftcode' ).value.length < 1 )
			{
				errorString += 'Bitte geben Sie den SWIFT-Code Ihrer Bank ein.<br/>';
			}
		}
		else
		{
...
function ShowEnlargedImagePreviewFromHighlightPlayer( id, HighlightPlayer )
{
	...
	var $BtnPrev = $J('<div/>', {'class': 'btnv6_blue_hoverfade btn_medium previous'}).append( $J('<span/>').text( 'Vorheriges' ) );
	var $BtnNext = $J('<div/>', {'class': 'btnv6_blue_hoverfade btn_medium next'}).append( $J('<span/>').text( 'Nächstes' ) );
	...
	{
		$ScreenshotCount.text( '%1$s von %2$s Bildern'.replace( /%1\$s/, iCurIndex + 1 ).replace( /%2\$s/, rgScreenshotURLs.length ) );
	};
...
{
	var waitDialog = ShowBlockingWaitDialog( 'Mitwirkende verwalten', 'Freundesliste wird geladen' );

	$J.get( "https://steamcommunity.com/sharedfiles/contributorpicker/" + publishedfileid,
		{},
		function( data, textStatus, jqXHR ) {
			waitDialog.Dismiss();
			gFriendsPicker = ShowAlertDialog( 'Mitwirkende verwalten', '<div class="friendsPicker" id="friendsPicker">' + data + '</div>', 'Abbrechen' );
		}
	);
}
			...
			return function(transport)
			{
				...
				switch ( responseJSON.success )
				{
					case 1:
						...
						break;
					default:
						ShowAlertDialog( 'Fehler', 'Das angegebene Profil konnte nicht gefunden werden.' );
						break;
				}
			}
		}(publishedfileid))
...
{
	if ( confirm( 'Möchten Sie die Liste der Mitwirkenden wirklich abschließen? Sie können nach diesem Schritt keine Mitwirkenden mehr hinzufügen oder entfernen.' ) != true )
	{
		return;
	}
...
function ShowExternalTagSelectorDialog( url, formID, submitFuncCB )
{
	...
	gExternalTagSelectorWaitDialog = ShowBlockingWaitDialog( 'Bitte warten …', 'Einstellungen werden geladen …' );
...
function PickWorkshopServiceProviders( publishedFileID, appID )
{
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxgetserviceproviders', {
		...
		var strTitle = 'Dienstanbieter auswählen';
		var strSaveChanges = 'Weiter';
...
function SaveWorkshopServiceProviders( publishedFileID )
{
	...
	{
		ShowAlertDialog( 'Fehler', 'Die Einnahmenprozentsätze der Dienstleister müssen zusammen 100 ergeben.' );
		return;
	}
	...
	$J.post( 'https://steamcommunity.com/sharedfiles/ajaxsetserviceprovidersplits', {
		...
		{
			case 14:
				ShowAlertDialog( 'Fehler', 'Sie können keine dritte Organisation wählen, die auch ein normaler Beitragender zu Ihrem Gegenstand ist.' );
				break;
			default:
				ShowAlertDialog( 'Fehler', 'Bei der Verarbeitung Ihrer Anfrage ist ein Fehler aufgetreten:' + ' ' + rgResults['success'] );
				break;
		}
		$J( "#SavingServiceProviderRevenueShares" ).hide();
	});
...
{
	var dialog = ShowConfirmDialog( 'Löschen', 'Möchten Sie diese Suchanfrage wirklich löschen?' );
	dialog.done( function() {
		$J.post( 'https://steamcommunity.com/sharedfiles/ajaxsavedwebquerydelete', { 'sessionid' : g_sessionID, 'appid': appid, 'queryid': queryid } )
		.done( function() {
			top.location.reload();
		} ).fail( function() {
			ShowAlertDialog( 'Fehler', 'Ihre gespeicherte Suchanfrage konnte nicht gelöscht werden. Bitte versuchen Sie es später erneut.' );
		} );
	} );
}
...
function SaveWorkshopSearchQuery( appid, params )
{
	$J.get( 'https://steamcommunity.com/sharedfiles/ajaxsavedwebqueriesget/', { appid: appid } )
		...
		if ( json.success != '1' )
		{
			ShowAlertDialog( 'Fehler', 'Ihre gespeicherten Suchanfragen konnten nicht abgerufen werden.' );
			return;
		}
		...
		var dialog = ShowPromptDialog( 'Ihre Suchanfrage speichern', 'Name Ihrer Suchanfrage:', 'Suchanfrage speichern', null, rgModalParams );
		...
		if ( json.queries && json.queries.length != 0 )
		{
			...
			container.append( $J( '<div/>', { text: 'Wählen Sie eine bestehende Suchanfrage zur Aktualisierung aus.' } ) );
			...
			var newOption = $J( '<option/>', { value: 0, text: '<Neue gespeicherte Suchanfrage>' } );
		...
		dialog.done( function( queryName ) {
			...
			$J.post( 'https://steamcommunity.com/sharedfiles/ajaxsavedwebqueryupdate', params )
				.done( function() {
					ShowAlertDialog( 'Gespeichert', 'Ihre Suchanfrage wurde gespeichert.' );
				} ).fail( function() {
				ShowAlertDialog( 'Fehler', 'Es gab ein Problem beim Speichern Ihrer Suchanfrage. Bitte versuchen Sie es später erneut.' );
			} );
		} );
